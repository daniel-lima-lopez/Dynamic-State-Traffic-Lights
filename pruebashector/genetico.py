...
class MyProblem(Problem):
	...
	def _evaluate(self, x, out, *args, ** kwargs):
		...
		for i in range(x.shape[0]):
			...
			if i == 0:
				red = SimStateNN(gen=x[i],steps=100, seed=self.gen, act_rou=True, gui=False, verbose=False, worst=max_steps)
			else:
				red = SimStateNN(gen=x[i],steps=100, seed=self.gen, gui=False, verbose=False, worst=max_steps, act_rou=False)
			red.run()
