...
class MyProblem(Problem):
	...
	def _evaluate(self, x, out, *args, ** kwargs):
		...
		for i in range(x.shape[0]):
			...
			if i == 0:
				red = SimStateNN(gen=x[i], seed=self.gen,steps=100, act_rou=True, gui=False, verbose=False, worst=max_steps)
			else:
				red = SimStateNN(gen=x[i], seed=self.gen, steps=100,gui=False, verbose=False, worst=max_steps, act_rou=False)
			red.run()
